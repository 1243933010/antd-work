...
export class EchartService {
  ...
  async create(createEchartDto: CreateEchartDto,user) {
    console.log(user)
    try {
      let data = await this.primsa.echartTable.findFirst({
        where:{
  ...
  async findAll(query,req) {
    let data = await this.primsa.echartTable.findMany({
      where:{
        userId:req.user.id,
        isShow:true,
  ...
  async update(id: number, updateEchartDto: UpdateEchartDto) {
    let result = await this.primsa.echartTable.update({
      where:{id},
  ...
  async remove(id: number) {
    let data = await this.primsa.echartTable.update({
      where:{
        id:id
      },
      data:{
